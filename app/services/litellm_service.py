...
async def call_litellm_embedding(user_id: str, service: str, model: str, input: list, options: dict = None):
    api_key = await get_api_key(user_id, service)
    if not api_key:
        raise Exception(f"No API key found for service: {service}")
    if service.lower() == "openai":
...
async def call_litellm_image_generation(user_id: str, service: str, model: str, prompt: str, options: dict = None):
    api_key = await get_api_key(user_id, service)
    if not api_key:
        raise Exception(f"No API key found for service: {service}")
    if service.lower() == "openai":
...
async def call_litellm_stream(user_id: str, service: str, model: str, messages: list, options: dict = None):
    api_key = await get_api_key(user_id, service)
    if not api_key:
        raise Exception(f"No API key found for service: {service}")
    if service.lower() == "openai":
...
def litellm_router(user_id: str, router_config: dict, messages: list, options: dict = None):
