...
async def litellm_stream(
    ...
    async def event_stream():
        async for chunk in call_litellm_stream(
            ...
            yield chunk["choices"][0]["delta"]["content"] if "choices" in chunk and chunk["choices"][0].get("delta") else ""
...
async def openai_proxy(request: Request):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post("http://localhost:4000/v1/chat/completions", content=await request.body(), headers=request.headers)
