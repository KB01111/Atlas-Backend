...
@router.post(
    "/", response_model=PluginConfigurationOut, status_code=status.HTTP_201_CREATED
)
async def create_plugin_config(
    config: PluginConfigurationCreate, user=Depends(get_current_user)
):
    try:
...
async def get_plugin_config(config_id: str, user=Depends(get_current_user)):
    try:
        ...
        if not config:
            raise HTTPException(
                status_code=404, detail="Plugin configuration not found"
            )
        return config
    except (PluginServiceError, SupabaseClientError) as e:
...
async def update_plugin_config(
    config_id: str, config: PluginConfigurationUpdate, user=Depends(get_current_user)
):
    try:
        updated = await PluginService.update_plugin_config(
            config_id, user["id"], config
        )
        if not updated:
            raise HTTPException(
                status_code=404, detail="Plugin configuration not found"
...
async def execute_plugin_config(
    config_id: str, inputs: dict, user=Depends(get_current_user)
):
    """
