...
@patch("app.services.workflow_service.WorkflowService.create_workflow", new_callable=AsyncMock, return_value={"id": "wf-1", "name": "Test Workflow", "user_id": "test-user-id", "description": "A test workflow", "steps": [], "created_at": None, "updated_at": None})
@pytest.mark.asyncio
...
@patch("app.services.workflow_service.WorkflowService.list_workflows", new_callable=AsyncMock, return_value=[])
@pytest.mark.asyncio
async def test_list_workflows(mock_list, async_client):
    response = await async_client.get("/api/v1/workflows/")
    assert response.status_code in (200, 401, 403)

@patch("app.services.workflow_service.WorkflowService.get_workflow", new_callable=AsyncMock, return_value=None)
@pytest.mark.asyncio
...
@patch("app.services.workflow_service.WorkflowService.update_workflow", new_callable=AsyncMock, return_value=None)
@pytest.mark.asyncio
...
@patch("app.services.workflow_service.WorkflowService.delete_workflow", new_callable=AsyncMock, return_value=None)
@pytest.mark.asyncio
...
@patch("app.services.workflow_service.WorkflowService.run_workflow", new_callable=AsyncMock, return_value={"status": "success", "output": {"message": "ok"}, "logs": []})
@pytest.mark.asyncio
...
@patch("app.services.workflow_service.WorkflowService.run_workflow", new_callable=AsyncMock, side_effect=Exception("Run failed"))
@pytest.mark.asyncio
